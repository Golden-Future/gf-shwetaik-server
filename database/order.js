...
let all = () => {
  return new Promise((resolve, reject) => {
    Order.aggregate([
      {
        $lookup: {
          from: "htable", // Look up the roles collection
          localField: "htable_id", // Match based on role_id
          foreignField: "htable_id",
          as: "table", // Output the results as "role"
        },
      },
      {
        $unwind: "$htable", // Unwind the role array to a single document
      },
    ]).exec((err, data) => {
      if (err) reject(err);
      resolve(data);
